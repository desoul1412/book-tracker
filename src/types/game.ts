 ...
 * the object. Members follow the task-spec names (lowercase snake_case).
 *
 ...
 *
 * Implemented as a const object so that both member-access syntax
 * (GameStatus.playing) and raw string literals ("RUNNING") satisfy the type.
 * This lets engine modules and tests use string literals directly without
 * a cast while UI components continue to use GameStatus.X for readability.
 */
...
export interface GameState {
  ...
  /** Queued direction change (applied at next tick to prevent 180-deg reversal). */
