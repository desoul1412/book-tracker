...
export interface CharacterRenderState {
  ...
  alertIcon?: '!' | '?' | '~' | null;  // ! phase/DLQ  ? budget throttle  ~ timeout
}
...
/** Color palette per alert icon type */
const ALERT_COLORS: Record<string, { glow: string; border: string; bg: string; text: string }> = {
  '!': { glow: '#ff2244', border: '#ff2244', bg: '#1a0005', text: '#ff2244' }, // phase complete / DLQ
  '?': { glow: '#ff8800', border: '#ff8800', bg: '#1a0800', text: '#ff8800' }, // budget throttled
  '~': { glow: '#ffcc00', border: '#ffcc00', bg: '#1a1400', text: '#ffcc00' }, // execution timeout
};

/** Render "!", "?", or "~" alert icon above a character sprite */
export function renderAlertIcon(
  ...
  const palette = ALERT_COLORS[char.alertIcon] ?? ALERT_COLORS['!'];
  const drawX = Math.round(char.pixelX);
  const drawY = Math.round(char.pixelY - (CHAR_FRAME_H - TILE_SIZE));

  // Bouncing animation using sine wave
  const bounce = Math.sin(frameCount * 0.12) * 3;
  const iconX = Math.round(drawX + CHAR_FRAME_W / 2);
  const iconY = Math.round(drawY - 22 + bounce);
  ...
  ctx.fillRect(iconX - boxW / 2, iconY - boxH / 2, boxW, boxH);
  ctx.strokeStyle = palette.border;
  ctx.lineWidth = 1;
  ctx.strokeRect(iconX - boxW / 2 + 0.5, iconY - boxH / 2 + 0.5, boxW - 1, boxH - 1);
  ...
  ctx.fillText(char.alertIcon, iconX, iconY + 1);
...
export function renderFrame(
  ...
  // Alert icons on top of everything
  for (const char of sorted) {
    renderAlertIcon(ctx, char, frameCount ?? 0);
  }
}
