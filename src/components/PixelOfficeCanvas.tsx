...
// ── Component ─────────────────────────────────────────────────────────────────
//
// CANVAS-AS-VIEW INVARIANT (do not break):
//   This component MUST only read data from its `company` prop (sourced from
//   the Zustand store by parents). It MUST NOT:
//     - Call fetch() for data APIs (static asset JSON is fine)
//     - Import or call supabase
//     - Import orchestratorApi, api.ts, or any server-side module
//     - Open modals directly — dispatch Zustand actions instead
//   This isolation means PixelOfficeCanvas can be fully replaced in v2 (Canvas v2
//   with A* pathfinding + diegetic props) without touching the data layer.
...
export function PixelOfficeCanvas({ company }: PixelOfficeCanvasProps) {
    ...
    for (const emp of employees) {
      ...
      switch (emp.status) {
        case 'working':
          ...
          // Prefer granular sub-status (e.g. "Reading foo.ts") over generic task name
          if (emp.activityDetail) {
            agent.speechBubble = emp.activityDetail.length > 25
              ? emp.activityDetail.slice(0, 23) + '…'
              : emp.activityDetail;
          } else {
      ...
      // Sync alert icon from employee data
  ...
  useEffect(() => {
    ...
    function loop(now: number) {
      ...
      ctx!.imageSmoothingEnabled = false;
      renderFrame(ctx!, layout!, charStates, assets!, tileColors.length > 0 ? tileColors : undefined, frameCountRef.current);
      ctx!.restore();
